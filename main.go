...
func (t *Turmite) Step(field Field) error {
	//read current color by using current coord
	color := field[t.y][t.x]
	//get action struct from t.rules map given color and state
	action := t.rules[Signal{state: t.state, color: color}]
	// follow action struct
		// change color to the int
	field[t.y][t.x] = action.color
	// change state of mite
	t.state = action.state
	// turn using turn functions
	if action.turn == RightDir{
		t.currentDir = t.currentDir.Right()
	} else if action.turn == SouthDir{
		t.currentDir = t.currentDir.Right()
		t.currentDir = t.currentDir.Right()
	} else if action.turn == LeftDir {
		t.currentDir = t.currentDir.Left()
	} else if action.turn != ForwardDir {
		fmt.Println(action.turn)
		return errors.New("bad turn")
	}
	// go 1 step in the correct direction
		// if for each north/south/...
	if t.currentDir == NorthDir {
		t.y--
	} else if t.currentDir == SouthDir{
		t.y++
	...
	// if t.x > len(field)-1 || t.y > len(field)-1{
	// 	return errors.New("out of bounds step")
	// }
		// if error return error // or do a mod flip
